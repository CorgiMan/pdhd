...
export const ref = (table) => varchar({ length: 30 }).notNull().references(() => table.id);
...
export function table(name, config) {
    return pgTable(name, {
        ...
        for (const field of Object.values(table)) {
            if (field.columnType !== "PgVarchar" || field.config.length !== 30 || !field.name.endsWith("Id")) continue;
